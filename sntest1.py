...
class MasterWindow:
    ...
    def Billing(self,box):
        if os.path.isfile("MSW.db") :
            if not bill.wincheck():
                bill.MainWindow()
                
            else:
                messagebox.showinfo(parent=box,title="Error",message="window already up")
                bill.MainWindow.bbox.lift()
            return
        else:
            messagebox.showinfo(parent=box,title="Error",message="Database file not found!!")
            return
    #Definiton for return product
    def ReturnProduct(self,box):
        if os.path.isfile("MSW.db"):
            ret.returnwin()
            return
        else:
